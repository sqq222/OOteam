...
class AbstractUser(AbstractBaseUser, PermissionsMixin):
    ...
    user_type = models.IntegerField(default=0, choices=USER_TYPE_CHOICES, blank=True, verbose_name='用户类型')
    ...
    store = models.ForeignKey(Stores, null=True, blank=True, verbose_name='门店')
    ...
    code = models.CharField(max_length=8, null=True, blank=True, verbose_name='用户编号')
    basemob_uuid = models.CharField(max_length=100, null=True, blank=True, verbose_name='环信UUID')
    img = models.ImageField(upload_to='user_img/', null=True, blank=True, verbose_name='头像')
    is_basemob = models.BooleanField(default=False, verbose_name='是否接收环信消息')
...
class FoodCategory(models.Model):
    ...
    is_gift = models.BooleanField(default=False, verbose_name='是否赠品分类')
...
class FoodSpec(models.Model):
    name = models.CharField(max_length=50, verbose_name="名称")
    price = models.DecimalField(default=0, max_digits=9, decimal_places=2, verbose_name="不含税价格")
    in_tax_price = models.DecimalField(default=0, max_digits=9, decimal_places=2, verbose_name="含税价格")
    vip_price = models.DecimalField(default=0, max_digits=9, decimal_places=2, verbose_name="会员不含税价格")
    in_tax_vip_price = models.DecimalField(default=0, max_digits=9, decimal_places=2, verbose_name="会员含税价格")
    tax_value = models.DecimalField(default=0, max_digits=9, decimal_places=2, verbose_name='税')
    vip_tax_value = models.DecimalField(default=0, max_digits=9, decimal_places=2, verbose_name='VIP税')
    ...
    def save(self, force_insert=False, force_update=False, using=None,
             update_fields=None):
        if self.food.is_include_tax:
            # 含税
            self.price = self.in_tax_price / (1 + (self.food.tax.tax / 100))
            self.vip_price = self.in_tax_vip_price / (1 + (self.food.tax.tax / 100))
        else:
            # 不含税
            self.in_tax_price = self.price * (1 + (self.food.tax.tax / 100))
            self.in_tax_vip_price = self.vip_price * (1 + (self.food.tax.tax / 100))

        self.tax_value = self.in_tax_price - self.price
        self.vip_tax_value = self.in_tax_vip_price - self.vip_price
        super(FoodSpec, self).save(force_insert=force_insert, force_update=force_update, using=using,
                                   update_fields=update_fields)

    @property
    def all_num(self):
        ret = Order_Food.objects.filter(food_spec_id=self.id, order__meal__status=0, status__lt=3,
...
class DeskCategory(models.Model):
    ...
    class Meta:
        verbose_name = "门店 - 餐桌类别"
        verbose_name_plural = "门店 - 餐桌类别"
...
class Meal(models.Model):
    ...
    store = models.ForeignKey(Stores, verbose_name='用餐店铺', related_name='meal_store')
    desk = models.ForeignKey(Desk, verbose_name='桌号')
    status = models.IntegerField(default=0, choices=STATUS_CHOICES, verbose_name='用餐状态')
    total_user = models.IntegerField(default=0, verbose_name='用餐人数')
    is_active = models.BooleanField(default=True, verbose_name='是否正常')

    @property
    def order_time(self):
        order = Order.objects.filter(meal_id=self.id)
        for o in order:
            if o.is_all_finish:
                continue
            else:
                return o.created
        return order.first().created
    ...
    def is_all_finish(self):
        ...
        :return:  True/False
        """
        order_food = Order_Food.objects.filter(order__meal__id=self.id)
        for o in order_food:
            if o.status < 3:
                return False
        return True
    ...
    @property
    def is_all_pay(self):
        order_count = Order.objects.filter(meal_id=self.id).count()
        pay_count = PayInfo.objects.filter(order__meal_id=self.id).count()
        if order_count == pay_count:
            return True
        else:
            return False
    ...
    @property
    def is_unpay(self):
        return True if Order.objects.filter(meal_id=self.id, status=1).count() > 0 else False
...
class Order(models.Model):
    ...
    code = models.CharField(null=True, blank=True, max_length=50, verbose_name='订单编号')
    total_price = models.DecimalField(default=0, max_digits=9, decimal_places=2, verbose_name="总价")
    total_vip_price = models.DecimalField(default=0, max_digits=9, decimal_places=2, verbose_name="VIP总价")
    created = models.DateTimeField(auto_now_add=True, verbose_name='创建时间')
    status = models.IntegerField(choices=STATUS_CHOICES, default=0, verbose_name='状态')
    is_active = models.BooleanField(default=True, verbose_name='是否正常')
    waiter = models.ForeignKey(AbstractUser, null=True, blank=True, verbose_name='服务员')
    ...
    def is_all_finish(self):
        ...
        :return: True/False
        """
        order_food = Order_Food.objects.filter(order_id=self.id)
        for o in order_food:
            if o.status < 3:
                return False
    ...
    @property
    def vip_tax_value(self):
        order_food = Order_Food.objects.filter(order_id=self.id)
        result = 0
        for o in order_food:
            result += o.vip_tax_value
        return result
...
class OrderFoodQuerySet(models.query.QuerySet):
    def delete(self):
        if self.count() > 0:
            order_id = self[0].order_id
            super(OrderFoodQuerySet, self).delete()
            order_food = Order_Food.objects.filter(order_id=order_id)
            total_price = 0
            for o in order_food:
                total_price += o.price
            total_vip_price = 0
            for o in order_food:
                total_vip_price += o.vip_price

            Order.objects.filter(id=order_id).update(total_price=total_price,
...
class Order_Food(models.Model):
    ...
    food_spec = models.ForeignKey(FoodSpec, verbose_name='商品规格', related_name='order_food_spec')
    num = models.IntegerField(verbose_name='数量')
    tag = models.ManyToManyField(Tag, verbose_name='标签', related_name='order_food_tag')
    price = models.DecimalField(max_digits=9, decimal_places=2, verbose_name="不含税价格")
    vip_price = models.DecimalField(max_digits=9, decimal_places=2, verbose_name="不含税VIP价格")
    ...
    def tax_value(self):
        ...
        #
        #     return round(
        #         (self.food_spec.price - (self.food_spec.price / (1 + self.food.tax.tax / 100))) * int(self.num),
        #         2)
        #
        # else:
        #     return round((self.food_spec.price * (self.food.tax.tax / 100)) * int(self.num), 2)

        return round(self.food_spec.tax_value * Decimal(self.num), 2)

    @property
    def tax_pay(self):
        # if self.food.is_include_tax:
        #     # 商品价格含税
        #     return round((self.food_spec.price - (self.food_spec.price / (1 + self.food.tax.tax / 100))) * self.num, 2)
        #
        # else:
        #     return round((self.food_spec.price * (self.food.tax.tax / 100)) * int(self.num), 2)
    ...
    def vip_tax_value(self):
        ...
        #     # 商品价格含税
        #     return round(
        #         (self.food_spec.vip_price - (self.food_spec.vip_price / (1 + self.food.tax.tax / 100))) * self.num, 2)
        # else:
        #     return round((self.food_spec.vip_price * (self.food.tax.tax / 100)) * int(self.num), 2)

        return round(self.food_spec.vip_tax_value * Decimal(self.num), 2)

    def delete(self, using=None, keep_parents=False):
        if self.is_gift:
            super(Order_Food, self).delete(using, keep_parents)
            return
        super(Order_Food, self).delete(using, keep_parents)

        order_food = Order_Food.objects.filter(order_id=self.order_id)
        total_price = 0
        for o in order_food:
            total_price += o.price
        order_food = Order_Food.objects.filter(order_id=self.order_id)
        ...
        Order.objects.filter(id=self.order_id).update(total_price=total_price,
                                                      total_vip_price=total_vip_price)

    def save(self, force_insert=False, force_update=False, using=None,
             update_fields=None):
        if self.num == 0:
            super(Order_Food, self).delete()
            return
        if self.is_gift:
            self.vip_price = 0
            self.price = 0
        ...
        self.vip_price = self.food_spec.vip_price * Decimal(self.num)
        super(Order_Food, self).save(force_insert, force_update, using, update_fields)
...
class Coupon(models.Model):
    ...
    desc = models.CharField(max_length=100, null=True, blank=True, verbose_name='备注')
    display_background = models.BooleanField(verbose_name='只显示背景图片和详情次')
    created = models.DateTimeField(auto_now_add=True, verbose_name='创建时间')
...
class FullMinusCoupon(models.Model):
    ...
    gift = models.ForeignKey(Food, blank=True, null=True, verbose_name='赠品')
...
class PayInfo(models.Model):
    ...
    coupon_code = models.OneToOneField(CouponCode, verbose_name='优惠券代码', null=True, blank=True,
                                       related_name='pay_coupon')
    coupon = models.ForeignKey(Coupon, verbose_name='优惠券', null=True, blank=True)
    ...
    money_pay = models.IntegerField(default=0, verbose_name='现金支付')
    created = models.DateTimeField(auto_now_add=True, verbose_name='支付时间')
    code = models.CharField(max_length=30, verbose_name='支付编号')
    limit_price = models.IntegerField(default=0, verbose_name='优惠金额')
    tax = models.DecimalField(default=0, max_digits=9, decimal_places=2, verbose_name='税')
    ...
    def save(self, force_insert=False, force_update=False, using=None,
        ...
        self.code = timezone.now().strftime(settings.CODE_DATETIME_FORMAT) \
                    + get_random_string(length=9, allowed_chars='0123456789')
        super(PayInfo, self).save(force_insert, force_update, using, update_fields)
        from shop.api.tasks import auto_meal_leave_payed
        auto_meal_leave_payed.apply_async((self.order.meal_id,), eta=datetime.utcnow() + timedelta(hours=5))
...
class Queue(models.Model):
    ...
    def __str__(self):
        return "{0} - 大人{1}人 - 小孩{2}人 - 禁烟:{3}".format(self.store.name,
                                                       self.num,
                                                       self.chindren_num,
                                                       self.no_smoking)
...
class StorePoint(models.Model):
    ...
    store = models.ManyToManyField(Stores, verbose_name='会员点可用店铺')
...
class StorePointLog(models.Model):
    ...
    point = models.ForeignKey(StorePoint, null=True, blank=True, verbose_name='门店')
    user = models.ForeignKey(AbstractUser, verbose_name='用户')
    pay_type = models.IntegerField(default=0, choices=PAY_TYPE_CHOICES, verbose_name='支付方式')
    ...
    end_date = models.DateField(verbose_name='到期时间')
    ...
    def __str__(self):
        return "{0} - {1} - {2}".format(self.point, self.recharge_point, self.created)
...
class GoldLog(models.Model):
    ...
    store = models.ForeignKey(Stores, verbose_name='充值店铺')
...
class RefundOrder(models.Model):
    ...
    refund_type = models.IntegerField(choices=TYPE_CHOICES, verbose_name='退款类型')
    code = models.CharField(null=True, blank=True, max_length=50, verbose_name='订单编号')
    user = models.ForeignKey(AbstractUser, verbose_name='用户')
    order = models.ForeignKey(Order, verbose_name='订单')
    store = models.ForeignKey(Stores, verbose_name='门店')
    food = models.ForeignKey(Order_Food, null=True, blank=True, verbose_name='商品')
    num = models.IntegerField(default=0, verbose_name='商品退货数量')
    refund_point = models.IntegerField(default=-1, verbose_name='退款会员点')
    refund_gold = models.IntegerField(default=-1, verbose_name='退款金币')
    refund_cash = models.IntegerField(default=-1, verbose_name='退款现金')
    reason = models.IntegerField(choices=REASON_CHOICES, verbose_name='理由')
    desc = models.TextField(verbose_name='描述')
    status = models.IntegerField(default=0, choices=STATUS_CHOICES, verbose_name='状态')
    refuse_desc = models.TextField(null=True, blank=True, verbose_name='拒绝理由')
    image1 = models.ImageField(upload_to='refund_order/', null=True, blank=True, verbose_name='图片1')
    image2 = models.ImageField(upload_to='refund_order/', null=True, blank=True, verbose_name='图片2')
    image3 = models.ImageField(upload_to='refund_order/', null=True, blank=True, verbose_name='图片3')
    image4 = models.ImageField(upload_to='refund_order/', null=True, blank=True, verbose_name='图片4')
    created = models.DateTimeField(auto_now_add=True, verbose_name='创建时间')
    process_date = models.DateTimeField(null=True, blank=True, verbose_name='处理时间')
    ...
    def save(self, force_insert=False, force_update=False, using=None,
        ...
        if self.refund_type == 0:
            ...
            order.status = 5
            order.save()
        super(RefundOrder, self).save(force_insert, force_update, using, update_fields)
    ...
    def __str__(self):
        return "{0} - {1}".format(self.order.code, self.get_status_display())


class StorePayType(models.Model):
    store = models.OneToOneField(Stores, verbose_name='店铺')
    ...
    class Meta:
        verbose_name = '门店 - 支付方式'
        verbose_name_plural = '门店 - 支付方式'
...
class UserGift(models.Model):
    store = models.ForeignKey(Stores, verbose_name='门店')
    food = models.ForeignKey(FoodSpec, verbose_name='商品规格')
    num = models.IntegerField(default=0, verbose_name='数量')
    user = models.ForeignKey(AbstractUser, verbose_name='用户')
    created = models.DateTimeField(auto_now_add=True, verbose_name='创建时间')
    is_use = models.BooleanField(default=False, verbose_name='是否使用')
    use_date = models.DateTimeField(null=True, blank=True, verbose_name='使用时间')
    payinfo = models.ForeignKey(PayInfo, null=True, blank=True, verbose_name='支付信息')
    ...
    def __str__(self):
        return '{0} {1} {2} {3}'.format(self.user.username, self.store, self.food, self.num)
